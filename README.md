...
| Hooks          | Husky + commitlint + lint-staged                                                                     | pre-commit: staged fixes + TDD sibling gate + repo-wide lint/format/typecheck; pre-push: `verify:ci` |
...
`HTTP 403 "Code scanning is not enabled for this repository."` from the upload step,
and the fix is to either enable Advanced Security or delete the `codeql` job and keep
`gitleaks` — not to weaken the workflow. The header of `security.yml` says the same
thing at the point of use.

Note what CodeQL cannot see here: `ios/` and `android/` are generated by
`expo prebuild` and never committed, so native code is outside the scanned tree by
construction. Native review belongs to the build pipeline, not to this workflow.
